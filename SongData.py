...
class SongData(object):
	...
	def constructRelativePitchDistanceVector(self, pitchMode, previousAction):
		...
		if previousAction == self.REST:										#previous action was rest bit (1 bit)
			...
			distanceTuple += (0,)
		if pitchMode == self.REST:											#pitchMode was rest bit (1 bit)
			distanceTuple += (1,)
			distanceExists = False
		else:
			distanceTuple += (0,)
	...
	def findaction(self,index,mode):
		try:
			#if index == 962:
			#	print('poop')
			...
			for i in [self.NULL, self.REST, self.END, self.HOLD]:
				if nextobservation == i:
					return i
			return mode-nextobservation 
		except IndexError:
