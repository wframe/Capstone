...
if __name__ == '__main__':
	...
	TOTALSONGSTOMONTECARLO = 1
	songs= []

	def incorporateSongIntoPolicy(node, policy):			
		parent = node.parent
	...
	while len(songs) < TOTALSONGSTOMONTECARLO:
		...
		try:
			...
			if terminal is not None:
				...
				incorporateSongIntoPolicy(terminal, policy)
		except RuntimeError:
	...
	def convertTrajectoryToMidi(songEnds):
		...
		startbeat = start.beatwithinmeasure
		while parent is not None:
			actions.append(node.actiongenerated) 
			node = parent
			parent = node.parent
		actions.append(start.actiongenerated)
		actions = actions[::-1]
		events = actionsToEvents(start.pitch, actions)
		valid = True
		for event in events:
